window.INTERVIEW_QA = [
  {
    ...
    category: "正则化",
    tags: ["L1", "L2", "正则化"],
    question: "L1、L2 正则化的原理讲一下？",
    answer: [
      "正则化的核心目的是限制模型复杂度，降低过拟合风险，做法是在原始损失函数后面加一个惩罚项。",
      "L1 正则化常写成 L = L_data + λΣ|w_i|；L2 正则化常写成 L = L_data + λΣw_i^2，其中 λ 是正则化系数。",
      "直观上，L1 倾向把一部分参数直接压到 0，L2 倾向把参数整体压小但通常不会直接变成 0。"
    ]
  },
  {
    id: "Q257",
    ...
    category: "正则化",
    tags: ["L1", "L2", "等高线"],
    question: "L1 和 L2 的区别，数学上解释一下，等高线怎么理解？",
    answer: [
      "数学形式上，L1 是参数绝对值之和，L2 是参数平方和。",
      "几何上常把损失函数等高线和正则约束区域一起看：L1 约束区域像菱形，L2 约束区域像圆。",
      "因为菱形在坐标轴尖角处更容易和等高线先接触，所以 L1 更容易让某些参数恰好变成 0，这就是稀疏性的来源。"
    ]
  },
  {
    id: "Q258",
    ...
    category: "正则化",
    tags: ["L1", "次梯度", "近端梯度"],
    question: "L1 正则不是连续可导的，那么还能用梯度下降么？如果不能的话如何优化求解？",
    answer: [
      "L1 在 0 点不可导，但不是不能优化。",
      "常见处理方式有三类：用次梯度，也就是 subgradient；用近端梯度法，也就是 proximal gradient；以及坐标下降法，也就是 coordinate descent。",
      "最稳的回答是：L1 虽然在 0 点不可导，但可以用次梯度替代；在大规模机器学习里更常见的是近端方法。"
    ]
  },
  {
    id: "Q259",
    ...
    topicGroup: "基础知识点",
    category: "正则化",
    tags: ["Dropout", "Early Stopping", "Data Augmentation"],
    question: "正则化有哪些种，分别有什么作用？",
    answer: [
      "常见正则化方法有 L1、L2、Dropout、Early Stopping，也就是早停、Data Augmentation，也就是数据增强，以及广义上带一点正则效果的 Batch Normalization。",
      "L1 促稀疏，能做特征选择；L2 抑制权重过大，让模型更平滑稳定；Dropout 通过随机丢神经元减少共适应；Early Stopping 防止模型在训练集上学得过头。",
      "一句话总结：凡是能降低过拟合、提升泛化的方法，广义上都可以看成正则化手段。"
    ]
  },
  {
    id: "Q260",
    ...
    topicGroup: "基础知识点",
    category: "正则化",
    tags: ["MAP", "高斯先验", "拉普拉斯先验"],
    question: "L1 和 L2 的区别？从贝叶斯估计角度看，它们的先验分布是什么？",
    answer: [
      "从贝叶斯角度看，正则化项可以理解成对参数加先验。",
      "L2 对应高斯先验，也就是参数服从正态分布；L1 对应拉普拉斯先验，也就是参数更容易集中在 0 附近。",
      "因为做 MAP，也就是 Maximum A Posteriori，中文叫最大后验估计时，对负对数后验最小化，高斯先验会导出平方项，拉普拉斯先验会导出绝对值项。"
    ]
  },
  {
    id: "Q261",
    ...
    category: "正则化",
    tags: ["L1", "稀疏解", "次梯度"],
    question: "L1 正则产生稀疏解的原因？0 点不可导怎么处理？",
    answer: [
      "L1 产生稀疏解的原因，一方面是几何上 L1 约束区域是菱形，更容易在坐标轴尖角处取到最优；另一方面是它对小权重也有持续拉回到 0 的趋势。",
      "0 点不可导的处理常见有三种：次梯度、近端梯度法、坐标下降法。",
      "如果面试官继续追问大规模问题怎么做，最稳的回答是实际里常用近端梯度或其变种来求解。"
    ]
  },
  {
    id: "Q262",
    ...
    category: "正则化与激活函数",
    tags: ["L1", "L2", "ReLU"],
    question: "L1、L2 的区别和应用场景？你知道哪些激活函数？为什么需要激活函数，它解决了什么问题？为什么 ReLU 比 sigmoid 更能缓解梯度消失？",
    answer: [
      "L1 适合希望模型稀疏、顺便做特征选择的场景；L2 更稳定，在大多数深度学习和传统机器学习里更常用。",
      "常见激活函数有 Sigmoid、Tanh、ReLU、Leaky ReLU、PReLU、ELU、GELU、Swish。之所以需要激活函数，是因为神经网络需要非线性；没有激活函数，多层线性层叠起来还是线性模型。",
      "ReLU 比 Sigmoid 更能缓解梯度消失，是因为 Sigmoid 两端会饱和、导数很小，而 ReLU 在正半轴导数恒为 1，不容易让梯度快速衰减。"
    ]
  },
  {
    id: "Q263",
    ...
    topicGroup: "基础知识点",
    category: "压缩剪枝量化加速",
    tags: ["剪枝", "Lasso", "Group Lasso"],
    question: "剪枝与正则化的联系，结构化剪枝和非结构化剪枝分别对应 Lasso 和 Group Lasso 的角度来回答",
    answer: [
      "剪枝和正则化的联系在于它们都在做模型压缩和复杂度控制。",
      "从稀疏约束角度看，非结构化剪枝更接近 Lasso，也就是 L1 正则，因为它鼓励单个权重变成 0；结构化剪枝更接近 Group Lasso，因为它按通道、卷积核、层等整组参数一起做稀疏。",
      "工程上，非结构化剪枝压缩率高但不一定真提速；结构化剪枝更容易在硬件上获得实际加速。"
    ]
  },
  {
    id: "Q264",
    ...
    topicGroup: "基础知识点",
    category: "压缩剪枝量化加速",
    tags: ["结构化剪枝", "非结构化剪枝", "加速"],
    question: "结构化剪枝和非结构化剪枝",
    answer: [
      "非结构化剪枝是直接删单个权重，稀疏度高，但普通硬件不一定能高效利用这种稀疏矩阵。",
      "结构化剪枝是按结构删除，比如通道剪枝、卷积核剪枝、层剪枝、注意力头剪枝。",
      "一句话总结：非结构化剪枝更像压参数，结构化剪枝更像提速度。"
    ]
  },
  {
    id: "Q265",
    ...
    category: "压缩剪枝量化加速",
    tags: ["蒸馏", "剪枝", "量化"],
    question: "三大角度：蒸馏、剪枝、量化",
    answer: [
      "蒸馏的英文全称是 Knowledge Distillation，中文叫知识蒸馏，核心思想是让小模型去模仿大模型的输出分布或中间特征。",
      "剪枝的核心是删掉不重要的参数、通道、卷积核或层，减少模型规模和计算量；量化的核心是把高精度表示改成低精度表示，比如 FP32 变成 INT8。",
      "三者对比可以概括为：蒸馏偏知识迁移，剪枝偏删结构，量化偏降精度，工程里经常组合使用。"
    ]
  },  {
    id: "Q242",
    ...
    category: "损失函数与优化",
    tags: ["交叉熵", "分类损失", "概率分布"],
    question: "讲下交叉熵的公式和意义",
    answer: [
      "交叉熵用来衡量预测分布和真实分布之间的差异。分类里常见写法是 L = -Σ y_i log p_i。",
      "如果真实标签是 one-hot，也就是只有正确类别是 1，其余是 0，那么公式就变成 L = -log p_true。",
      "它的意义很直接：模型给正确类别的概率越大，损失越小；给正确类别的概率越小，损失越大。"
    ]
  },
  {
    id: "Q243",
    ...
    topicGroup: "基础知识点",
    category: "损失函数与优化",
    tags: ["交叉熵", "Softmax", "MSE"],
    question: "为什么分类问题的损失函数常用交叉熵？",
    answer: [
      "因为分类输出本身就是概率分布，交叉熵正好用来比较预测分布和真实分布是否一致。",
      "它比 MSE，也就是 Mean Squared Error，中文叫均方误差，更适合分类，因为它对错误且高置信度的预测惩罚更强。",
      "同时它和 Softmax 或 Sigmoid 配合时梯度性质更好，所以分类问题里通常优先用交叉熵。"
    ]
  },
  {
    id: "Q244",
    ...
    question: "激活函数作用，有哪些，都怎么改进？",
    answer: [
      "激活函数的作用是给网络引入非线性。如果没有激活函数，多层线性层叠起来仍然只是线性变换。",
      "常见激活函数有 Sigmoid、Tanh、ReLU、Leaky ReLU、PReLU、ELU、GELU、Swish。",
      "它们的改进方向一般是缓解梯度消失、避免 ReLU 神经元死亡、让函数更平滑，以及让训练更稳定。"
    ]
  },
  {
    id: "Q245",
    ...
    topicGroup: "基础知识点",
    category: "激活函数",
    tags: ["ReLU", "激活函数", "Dying ReLU"],
    question: "ReLU 函数所有背景、原理、应用？",
    answer: [
      "ReLU 的英文全称是 Rectified Linear Unit，中文叫修正线性单元，公式是 ReLU(x) = max(0, x)。",
      "它出现的背景是早期 Sigmoid 和 Tanh 在深层网络里容易梯度消失，所以需要一种更简单、更不容易饱和的激活函数。",
      "ReLU 的优点是计算简单、正半轴梯度恒为 1、不容易梯度消失；缺点是负半轴恒为 0，可能出现 Dying ReLU。它广泛用在 CNN 和 MLP 的隐藏层。"
    ]
  },
  {
    id: "Q246",
    ...
    topicGroup: "基础知识点",
    category: "激活函数",
    tags: ["ReLU", "Sigmoid", "梯度消失"],
    question: "ReLU 和 Sigmoid 相比，优点有哪些？ReLU 解决了什么问题？",
    answer: [
      "ReLU 相比 Sigmoid 的优点是计算更简单、正半轴不容易梯度消失、收敛通常更快，也更适合深层网络。",
      "它主要解决的是 Sigmoid 在深层网络里容易饱和、梯度太小、训练慢的问题。",
      "但 ReLU 也不是完美的，它会带来神经元死亡问题，所以后来有 Leaky ReLU 等改进。"
    ]
  },
  {
    id: "Q247",
    ...
    topicGroup: "基础知识点",
    category: "网络优化梯度下降",
    tags: ["梯度下降", "牛顿法", "Hessian"],
    question: "梯度下降、牛顿法优缺点？",
    answer: [
      "梯度下降只用一阶导数，也就是梯度，沿负梯度方向更新参数。优点是实现简单、适合大规模问题；缺点是收敛速度可能慢，对学习率敏感。",
      "牛顿法除了梯度，还用二阶导数信息，也就是 Hessian 矩阵。优点是局部收敛快；缺点是 Hessian 的计算和求逆代价很高。",
      "一句话总结：梯度下降便宜但可能慢，牛顿法更快但太贵。"
    ]
  },
  {
    id: "Q248",
    ...
    topicGroup: "基础知识点",
    category: "网络优化梯度下降",
    tags: ["BGD", "SGD", "Mini-batch"],
    question: "梯度下降和随机梯度下降讲一下？",
    answer: [
      "梯度下降一般指 BGD，也就是 Batch Gradient Descent，每次用全部训练样本算梯度再更新。",
      "随机梯度下降是 SGD，也就是 Stochastic Gradient Descent，每次只用一个样本或一个小批次更新。",
      "BGD 梯度更稳定但每步很慢；SGD 每步更便宜、更新更快，但噪声更大。工程里最常用的是 Mini-batch SGD。"
    ]
  },
  {
    id: "Q249",
    ...
    category: "网络优化梯度下降",
    tags: ["SGD", "Adam", "优化器"],
    question: "SGD、Adam 区别",
    answer: [
      "Adam 的英文全称是 Adaptive Moment Estimation，中文常叫自适应矩估计。",
      "它会同时利用一阶矩和二阶矩，也就是梯度均值和梯度平方均值，来做自适应学习率更新；SGD 则更简单，通常只配合固定或调度学习率。",
      "一般来说 Adam 收敛更快、更省调参；SGD 往往在最终泛化上更稳。"
    ]
  },
  {
    id: "Q250",
    ...
    topicGroup: "基础知识点",
    category: "网络优化梯度下降",
    tags: ["SGD", "BGD", "Momentum"],
    question: "SGD 和 BGD 区别？还知道哪些优化算法？动量的作用是什么？",
    answer: [
      "SGD 每次只看一个样本或小批次，BGD 每次看全量数据。前者更新快但有噪声，后者稳定但慢。",
      "常见优化算法还有 Momentum、Adagrad、RMSProp、Adam、AdamW。",
      "动量的作用是给优化加惯性，在方向一致时加速下降，在震荡方向上做平滑，也更容易穿过一些浅局部坑。"
    ]
  },
  {
    id: "Q251",
    ...
    category: "网络优化梯度下降",
    tags: ["学习率", "梯度方向", "优化"],
    question: "梯度下降过程中如果不按正确方向进行怎么办？",
    answer: [
      "这通常说明更新方向有问题，常见原因是学习率太大、梯度噪声太强、损失面不平滑，或者梯度实现本身有错。",
      "常见处理方式有降低学习率、换更稳的优化器比如 Adam、加动量、做梯度裁剪、检查梯度计算和特征归一化。",
      "一句话总结：方向不对，先查梯度，再查学习率和优化器。"
    ]
  },
  {
    id: "Q252",
    ...
    category: "网络优化梯度下降",
    tags: ["优化器", "BGD", "AdamW"],
    question: "用什么优化方法，梯度下降的种类，各有什么优点？",
    answer: [
      "梯度下降常见三类是 BGD、SGD 和 Mini-batch GD。BGD 稳定但慢，SGD 快但噪声大，Mini-batch 最常用，兼顾效率和稳定。",
      "优化器方面，Momentum 擅长加速和减震荡，Adagrad 适合稀疏特征，RMSProp 缓解学习率衰减过快，Adam 收敛快，AdamW 把权重衰减和梯度更新解耦得更规范。",
      "工程里一般先从 Adam 或 SGD + Momentum 开始。"
    ]
  },
  {
    id: "Q253",
    ...
    category: "网络优化梯度下降",
    tags: ["梯度下降", "牛顿法", "约束优化"],
    question: "梯度下降法和牛顿法怎么实现的。优化问题分哪些种，无约束的优化问题怎么处理，有约束的优化问题怎么处理。",
    answer: [
      "梯度下降的更新公式是 θ_(t+1) = θ_t - η∇f(θ_t)，其中 η 是学习率。",
      "牛顿法的更新公式是 θ_(t+1) = θ_t - H^(-1)∇f(θ_t)，其中 H 是 Hessian，也就是二阶导矩阵。",
      "优化问题可以分无约束和有约束。无约束问题直接优化目标函数；有约束问题常用拉格朗日乘子法、KKT 条件、投影梯度法等方法处理。"
    ]
  },
  {
    id: "Q254",
    ...
    category: "网络优化梯度下降",
    tags: ["凸优化", "拟牛顿法", "BFGS"],
    question: "凸优化了解吗？牛顿法、SGD、最小二乘，各自的优势，牛顿法与 SGD 的区别？牛顿法能用于非凸函数吗？拟牛顿法说说呢？",
    answer: [
      "凸优化指目标函数是凸函数，凸问题的好处是局部最优就是全局最优。",
      "最小二乘在线性回归这类问题里简单直接；SGD 适合大规模数据和深度学习；牛顿法局部收敛快，适合中小规模高精度优化。",
      "牛顿法和 SGD 的核心区别是牛顿法用二阶信息，SGD 只用一阶信息。牛顿法也能用于非凸函数，但 Hessian 可能不是正定的；拟牛顿法比如 BFGS、L-BFGS，用近似 Hessian 来降低代价。"
    ]
  },
  {
    id: "Q255",
    ...
    topicGroup: "基础知识点",
    category: "网络优化梯度下降",
    tags: ["牛顿法", "深度学习", "Hessian"],
    question: "说一下牛顿法，为什么深度学习很少用牛顿法？牛顿法一般应用于什么场景，有什么好处？",
    answer: [
      "牛顿法是利用目标函数的一阶和二阶信息做更新的方法，在最优点附近通常收敛很快。",
      "深度学习里很少直接用牛顿法，主要因为参数维度太高，Hessian 计算和存储都太贵，求逆更贵，而且深度学习常常还是非凸问题。",
      "它更适合中小规模、对精度要求高的传统优化问题。它的好处是局部收敛速度快、迭代步数往往更少。"
    ]
  },  {
    id: "Q227",
    ...
    category: "其他方面",
    tags: ["梯度消失", "过拟合", "正则化"],
    question: "简单谈一下对梯度消失和如何防止过拟合的看法？",
    answer: [
      "梯度消失指的是反向传播时，越靠前层拿到的梯度越小，导致前面层几乎学不动。它常见于网络太深、激活函数饱和，比如 sigmoid，或者权重初始化不合适的情况。",
      "防止过拟合常见做法有：增加数据量、做数据增强、加正则化、Dropout、Early Stopping，也就是早停、以及控制模型复杂度。",
      "一句话总结：梯度消失是优化问题，过拟合是泛化问题，前者解决训练不动，后者解决测试不好。"
    ]
  },
  {
    id: "Q228",
    ...
    category: "其他方面",
    tags: ["CNN", "DNN", "参数共享"],
    question: "CNN 为什么比 DNN 在图像识别上更好？",
    answer: [
      "CNN 的英文全称是 Convolutional Neural Network，中文是卷积神经网络；DNN 的英文全称是 Deep Neural Network，中文一般指深度神经网络，这里通常是全连接网络。",
      "CNN 更适合图像，核心原因有三点：第一，局部连接更符合图像局部相关性；第二，参数共享让参数量更小；第三，它保留了空间结构信息。",
      "而全连接 DNN 会把图像直接拉平成向量，空间结构容易丢掉，参数量也会大很多，所以在图像任务里通常不如 CNN 高效。"
    ]
  },
  {
    id: "Q229",
    ...
    category: "其他方面",
    tags: ["反向传播", "卷积", "池化"],
    question: "神经网络的反向传播机制是什么？卷积和 pooling 怎么反传？",
    answer: [
      "反向传播本质上就是链式法则。先从损失函数对输出求导，再一层一层往前算，把梯度传回去，同时得到每层参数的梯度。",
      "卷积层反传通常记三件事：对输入求梯度、对卷积核权重求梯度、对偏置求梯度，本质还是局部区域上的链式求导。",
      "Pooling 如果是 Max Pooling，梯度只传给前向时最大值所在位置；如果是 Average Pooling，梯度平均分配给窗口里的元素。"
    ]
  },
  {
    id: "Q230",
    ...
    topicGroup: "基础知识点",
    category: "其他方面",
    tags: ["BN", "归一化", "Batch Normalization"],
    question: "解释一下 BN？为什么用 BN 层，归一化的操作是什么？",
    answer: [
      "BN 的英文全称是 Batch Normalization，中文叫批归一化。它是在一个 mini-batch 上把特征先标准化，再做可学习的缩放和平移。",
      "公式上通常写成先减均值、再除以标准差：x_hat = (x - mu) / sqrt(sigma^2 + epsilon)，然后再做 y = gamma * x_hat + beta。",
      "为什么要用 BN？主要是让训练更稳定、允许更大学习率、加快收敛，同时对梯度传播也更友好。"
    ]
  },
  {
    id: "Q231",
    ...
    category: "其他方面",
    tags: ["梯度爆炸", "梯度消失", "初始化"],
    question: "梯度爆炸和梯度消失的原因是什么？",
    answer: [
      "梯度消失通常是反向传播连乘很多小于 1 的数，导致梯度越来越小；梯度爆炸则是连乘很多大于 1 的数，导致梯度越来越大。",
      "常见诱因有：网络太深、激活函数饱和、权重初始化不合理、循环结构过长等。",
      "对应办法一般是：合理初始化、用 ReLU 这类更稳定的激活函数、加 BN、做梯度裁剪，也就是 Gradient Clipping。"
    ]
  },
  {
    id: "Q232",
    ...
    topicGroup: "基础知识点",
    category: "其他方面",
    tags: ["sigmoid", "激活函数", "非线性"],
    question: "神经网络为什么要加 sigmoid 函数？",
    answer: [
      "先说本质，不是一定要加 sigmoid，而是神经网络需要非线性激活函数。如果每层都只是线性变换，多层叠起来仍然等价于一层线性变换，表达能力很弱。",
      "sigmoid 的英文就是 Sigmoid Function，中文常叫 S 形激活函数。它会把输出压到 0 到 1 之间，早期常用在二分类输出层，因为可以解释成概率。",
      "但隐藏层里现在更常用 ReLU，因为 sigmoid 容易饱和，导致梯度消失。"
    ]
  },
  {
    id: "Q233",
    ...
    question: "如何防止过拟合？",
    answer: [
      "常见方法有：增加数据量、做数据增强、L1/L2 正则化、Dropout、Early Stopping，也就是早停、模型简化、交叉验证。",
      "如果是计算机视觉任务，数据增强尤其常见，比如随机裁剪、翻转、颜色抖动、Mixup、Mosaic。"
    ]
  },
  {
    id: "Q234",
    ...
    question: "初始学习率怎么设？",
    answer: [
      "学习率没有绝对固定值，但可以结合优化器、Batch Size 和模型规模来定。工程里常先从经验值起步，比如 SGD 常见从 0.1、0.01 试；Adam 常见从 1e-3 开始。",
      "更稳的答法是：先用经验初值，再看 loss 是否震荡、是否下降太慢，然后配合 warmup 和学习率衰减策略一起调。"
    ]
  },
  {
    id: "Q235",
    ...
    category: "其他方面",
    tags: ["迁移学习", "微调", "预训练"],
    question: "迁移学习是什么？怎么迁移？怎么选择迁移的模型？",
    answer: [
      "迁移学习就是把一个任务上学到的知识迁到另一个相关任务上。最常见做法是用预训练模型，再做 fine-tuning，也就是微调。",
      "怎么迁移一般有两种：一种是冻结骨干网络，只训练最后几层；另一种是加载预训练权重后整体微调。",
      "选模型时主要看三点：第一，源任务和目标任务是否接近；第二，模型规模和部署约束是否匹配；第三，预训练数据和目标数据的领域差距大不大。"
    ]
  },
  {
    id: "Q236",
    ...
    topicGroup: "基础知识点",
    category: "其他方面",
    tags: ["Softmax", "Logistic", "多分类"],
    question: "softmax 和多个 logistic 各自的优势是什么？",
    answer: [
      "Softmax 的英文全称就是 Softmax，中文一般直接叫 Softmax；Logistic 通常指 Logistic Regression，中文叫逻辑回归。",
      "Softmax 更适合单标签多分类，因为所有类别会一起归一化，输出可以看成互斥类别上的概率分布。",
      "多个 Logistic 更适合多标签分类，因为每个类别独立做二分类，一个样本可以同时属于多个类。"
    ]
  },
  {
    id: "Q237",
    ...
    category: "其他方面",
    tags: ["梯度消失", "ResNet", "BN"],
    question: "深度学习里面解决梯度消失的办法有哪些？",
    answer: [
      "常见办法有：用 ReLU 系列激活函数、合理初始化比如 Xavier 或 He 初始化、加 BN、使用残差连接比如 ResNet、在循环网络里用 LSTM 或 GRU 这类门控结构。",
      "一句话总结：核心思路就是让梯度更容易传、不要在层间连乘时越乘越小。"
    ]
  },
  {
    id: "Q238",
    ...
    category: "其他方面",
    tags: ["数据增强", "CV", "增强"],
    question: "CV 中做数据增强的方法有哪些？",
    answer: [
      "CV 是 Computer Vision，中文叫计算机视觉。常见数据增强有随机裁剪、缩放、旋转、平移、翻转、颜色抖动、加噪声、Cutout、Mixup、Mosaic。",
      "检测任务里还会用随机改变框、随机仿射变换等；分割任务要注意图像和标注要同步变换。"
    ]
  },
  {
    id: "Q239",
    ...
    category: "其他方面",
    tags: ["卷积优化", "工程优化", "推理加速"],
    question: "工程上如何对卷积操作进行优化？",
    answer: [
      "常见优化方向有：算子融合，比如 Conv + BN + ReLU 融合；选择高效实现，比如 Winograd 或 GEMM；做量化、剪枝、蒸馏；以及针对硬件做并行和访存优化。",
      "工程题里可以强调一句：理论 FLOPs 下降不等于真实延迟一定下降，真正瓶颈还包括访存和算子调度。"
    ]
  },
  {
    id: "Q240",
    ...
    category: "其他方面",
    tags: ["样本不均衡", "重采样", "损失函数"],
    question: "样本不均衡怎么处理？一个 batch 类别均等采样，修改 loss 对不同样本的权重。",
    answer: [
      "处理样本不均衡常见有三类方法：第一，数据层面做过采样、欠采样、分层采样；第二，损失函数层面做类别加权、Focal Loss；第三，评估层面不要只看准确率，还要看 Precision、Recall、F1、AUC。",
      "一个 batch 里做类别均衡采样是可行的，属于采样策略；给不同类别样本加不同 loss 权重也是常见做法。"
    ]
  },
  {
    id: "Q241",
    ...
    category: "其他方面",
    tags: ["学习率", "训练不稳定", "优化"],
    question: "学习率过大会出现什么问题，怎么解决？",
    answer: [
      "学习率过大常见现象是 loss 大幅震荡、训练不收敛，严重时甚至会数值溢出。",
      "解决办法一般是：降低学习率、加 warmup、用学习率衰减策略、检查初始化和 Batch Size，必要时配合梯度裁剪。"
    ]
  },  {
    id: "Q218",
    updatedAt: "2026-04-07",
  ...
  },
  {
    ...
    category: "卷积神经网络",
    tags: ["卷积", "CNN", "局部连接"],
    question: "卷积的作用是什么？它的局限有哪些？",
    answer: [
      "卷积的作用可以概括成四点：第一，提局部特征；第二，参数更少；第三，参数共享；第四，保留空间结构。",
      "图像里的边缘、纹理、角点这类信息本来就是局部模式，所以卷积很适合抓这种局部结构。相比全连接，卷积核更小，参数量会大幅下降。",
      "局限也要会答：它天然更擅长局部建模，长距离依赖建模不如注意力机制直接；标准卷积对旋转和尺度变化也不是天然不变，很多时候还要靠数据增强或专门结构补。",
      "一句话总结：卷积强在局部模式提取和参数高效，弱在全局依赖和几何变化建模。"
    ]
  },
  {
    id: "Q213",
    ...
    topicGroup: "基础知识点",
    category: "卷积神经网络",
    tags: ["VGG", "LeNet", "3x3卷积"],
    question: "比较 VGG 和 LeNet，VGG 使用 3×3 卷积核最大的优势是什么？",
    answer: [
      "LeNet 更早、更浅，结构也更简单，面向的是比较小规模、低分辨率任务；VGG 更深，卷积层堆得更多，表达能力明显更强。",
      "VGG 大量用 3×3 卷积，最大优势有三点：第一，多个 3×3 堆叠后感受野可以接近大卷积核；第二，参数更少；第三，非线性更多，因为中间多了一层激活函数，所以表达能力更强。",
      "一句话面试版：VGG 用 3×3 的核心优势是参数更省、非线性更多，而且堆叠后也能获得大感受野。"
    ]
  },
  {
    id: "Q214",
    ...
    category: "卷积神经网络",
    tags: ["CNN", "卷积", "池化"],
    question: "CNN 的核心是什么？卷积操作怎么进行？卷积反向传播和池化反向传播怎么理解？",
    answer: [
      "CNN 的英文全称是 Convolutional Neural Network，中文叫卷积神经网络。它的核心可以概括成：局部连接、参数共享、层级特征提取。",
      "卷积前向过程就是卷积核在输入特征图上滑动，每到一个位置就取一个局部窗口，和卷积核逐元素相乘再求和，得到输出特征图上的一个值。卷积核本质上可以理解成一个模式探测器。",
      "卷积反向传播一般记三件事：对输入求梯度、对卷积核权重求梯度、对偏置求梯度，本质上还是链式法则。",
      "池化前向过程里，Max Pooling 取窗口最大值，Average Pooling 取窗口平均值。反向传播时，如果是 Max Pooling，梯度只传给前向时取到最大值的那个位置；如果是 Average Pooling，梯度平均分给窗口里的所有元素。",
      "一句话总结：卷积负责提特征，池化负责压缩和增强鲁棒性，反向传播本质上都是链式法则。"
    ]
  },
  {
    id: "Q215",
    ...
    category: "卷积神经网络",
    tags: ["参数共享", "卷积核", "CNN"],
    question: "CNN 参数共享是什么意思？",
    answer: [
      "参数共享就是同一个卷积核参数，在整张特征图所有位置反复使用，而不是每个位置都学一套新的参数。",
      "这样做的好处有两个：第一，参数量大幅减少；第二，如果某种模式，比如边缘、纹理，出现在不同位置，同一个卷积核都能把它检测出来。",
      "所以参数共享本质上是一种同一种检测器在全图复用的思想。"
    ]
  },
  {
    id: "Q216",
    ...
    category: "卷积神经网络",
    tags: ["转置卷积", "上采样", "反卷积"],
    question: "反卷积和上采样是什么意思？",
    answer: [
      "上采样就是把特征图尺寸变大，比如从 32×32 变成 64×64，常见方式有最近邻插值和双线性插值。",
      "反卷积更准确的名字其实是转置卷积，英文全称是 Transposed Convolution，中文叫转置卷积。它不是严格意义上的逆卷积，而是普通卷积在线性代数形式下的转置操作，常用于可学习的上采样。",
      "区别可以这样说：普通上采样一般是固定规则，不带学习参数；转置卷积是可学习的，表达能力更强，但更容易出现棋盘格伪影。"
    ]
  },
  {
    id: "Q217",
    ...
    topicGroup: "基础知识点",
    category: "卷积神经网络",
    tags: ["Dropout", "正则化", "验证集"],
    question: "Dropout 跟浅层网络区别是什么？Dropout 后验证集如何处理？",
    answer: [
      "这个题其实分两部分。第一部分，Dropout 不是网络结构，它是一种正则化方法；浅层网络说的是模型深度，这是两个不同维度，不能直接对等比较。浅层网络是模型容量变小，Dropout 是训练时随机屏蔽一部分神经元，降低共适应。",
      "第二部分，验证集和测试集怎么处理：训练时用 Dropout；验证和测试时一般关闭 Dropout，也就是不再随机丢神经元，直接用完整网络做前向。现代框架里通常切到 eval() 模式就可以。",
      "一句话总结：Dropout 是训练阶段的正则化手段，验证和测试阶段通常关闭。"
    ]
  },
  {
    id: "Q203",
  ...
  },
  {
    ...
    answer: [
      "这三个东西容易混，因为都长得有点抽象。先别背定义，先抓它们分别是干嘛的。",
      "迭代器可以理解成一个每次给你下一个值的对象。你问它一次，它给你一个值；再问一次，它再给你下一个；直到没有了为止。",
      "生成器其实是一种更方便写迭代器的方法。带 yield 的函数不会一次性执行完，而是执行到 yield 先停住，把值吐出去，下次再从停住的地方继续。所以生成器是一种特殊的迭代器。",
      "装饰器和前两个不是一类东西。装饰器本质上是一个接收函数、再返回一个新函数的东西。它的作用一般是不改原函数代码，但给原函数额外加功能，比如日志、权限、缓存。",
      "最简单区分：迭代器是一个一个取值的对象，生成器是一种更容易写出来的迭代器，装饰器是给函数加功能的包装器。",
      "面试里可以直接这样答：迭代器是实现按顺序逐个返回元素的对象；生成器是用 yield 写出来的一种特殊迭代器，它会在执行过程中暂停并保留状态；装饰器本质上是接收函数并返回新函数的包装器，用来在不改原函数代码的前提下增强功能。"
    ]
  },
  {
    id: "Q210",
